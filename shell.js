...
const LS_COUNTDOWN_IDS  = 'countdown_ids';
const ID_FM_FRAME       = 'frame-flowmodoro';
const SEL_CD_FRAMES     = '.frame-countdown';
const MSG_CFG_UPDATE    = 'cfg-update';
const ID_COUNTDOWN_AREA = 'countdown-area';
const CD_WRAPPER_PFX    = 'cd-wrapper-';
const FRAME_CD_PFX      = 'frame-countdown-';
...
function getFlowmodoroFrame() {
  return document.getElementById(ID_FM_FRAME);
}
...
function saveCfg(key, val, min = 1) {
  ...
  if (!isNaN(n) && n >= min) {
    ...
    postToFlowmodoro({ type: MSG_CFG_UPDATE });
  }
}

// ── Click-outside: desactiva iframes para que los clicks lleguen al document
function disableIframePointerEvents() {
  document.getElementById(ID_FM_FRAME).style.pointerEvents = 'none';
  document.querySelectorAll(SEL_CD_FRAMES).forEach(f => f.style.pointerEvents = 'none');
}
function restoreIframePointerEvents() {
  document.getElementById(ID_FM_FRAME).style.pointerEvents = '';
  document.querySelectorAll(SEL_CD_FRAMES).forEach(f => f.style.pointerEvents = '');
}
...
let cdIds = JSON.parse(localStorage.getItem(LS_COUNTDOWN_IDS) || '["0"]');

function addCountdownFrame(id, isNew = false) {
  const wrapper = document.createElement('div');
  wrapper.className = 'countdown-wrapper';
  wrapper.id = `${CD_WRAPPER_PFX}${id}`;

  const iframe = document.createElement('iframe');
  iframe.id        = `${FRAME_CD_PFX}${id}`;
...
addBtn.addEventListener('click', () => {
  ...
  cdIds.push(newId);
  localStorage.setItem(LS_COUNTDOWN_IDS, JSON.stringify(cdIds));
...
window.addEventListener('message', (e) => {
  ...
  if (e.data.type === 'cfg-loaded') {
    ...
    const frame = document.getElementById(`${FRAME_CD_PFX}${e.data.id}`);
    if (frame) frame.style.height = e.data.height + 'px';
  } else if (e.data.type === 'history-open') {
    document.getElementById(ID_COUNTDOWN_AREA).hidden = true;
  } else if (e.data.type === 'history-close') {
    document.getElementById(ID_COUNTDOWN_AREA).hidden = false;
  } else if (e.data.type === 'countdown-remove') {
    const id = e.data.id;
    cdIds = cdIds.filter(x => x !== id);
    localStorage.setItem(LS_COUNTDOWN_IDS, JSON.stringify(cdIds));
    const wrapper = document.getElementById(`${CD_WRAPPER_PFX}${id}`);
